...
logging.basicConfig(
    ...
    format='%(message)s',
    handlers=[
        logging.FileHandler('cot_tools_server.log', mode='w'),
        logging.StreamHandler(sys.stderr)  # Use stderr for terminal output
    ]
)

# Create console for both stdout and stderr
console_stdout = Console(file=sys.stdout)
console_stderr = Console(file=sys.stderr)

def log_print(*args, **kwargs):
    """Print to both console and log file"""
    console_stdout.print(*args, **kwargs)
    console_stderr.print(*args, **kwargs)
    sys.stderr.flush()
    sys.stdout.flush()
'''

#console = Console()
# Create a file console for logging in overwrite mode
console = Console(file=open("cot_tools_server.log", "w", encoding='utf-8'))  # 'w' mode overwrites the file
